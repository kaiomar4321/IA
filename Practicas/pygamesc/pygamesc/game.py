...
mover_derecha_activado = False  # Variable para controlar si la tecla derecha está presionada
...
def mover_derecha():
    global jugador
    jugador.x += 5  # Mueve al jugador 5 píxeles a la derecha en cada ciclo
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_a:
                    ...
                    graficar_datos()  # Llamada a la función de graficado
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_SPACE and en_suelo and not pausa:  # Detectar la tecla espacio para saltar
                    salto = True
                    en_suelo = False
                if evento.key == pygame.K_RIGHT:  # Detectar la tecla derecha para mover el jugador
                    mover_derecha()
                if evento.key == pygame.K_LEFT:  # Detectar la tecla derecha para mover el jugador
                    mover_izquierda()
